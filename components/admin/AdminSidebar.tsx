...
export function AdminSidebar() {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-5 border-b border-rose-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-black text-[13px] leading-none" style={{ background: 'linear-gradient(135deg,#f43f5e,#ec4899)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Crochet
            </p>
      ...
      <div className="px-3 py-4 border-t border-rose-100 space-y-1">
        ...
        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-red-400 hover:bg-red-50 hover:text-red-600 transition-all">
  ...
  return (
    <>
      {/* Mobile toggle */}
      <button onClick={() => setOpen(true)} className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-white rounded-xl shadow-md flex items-center justify-center text-rose-500 border border-rose-100">
        <Menu size={20} />
      </button>
      ...
      <div className={`lg:hidden fixed top-0 left-0 h-full w-64 bg-white z-50 shadow-xl border-r border-rose-100 transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <button onClick={() => setOpen(false)} className="absolute top-4 right-4 p-2 text-rose-400 hover:bg-rose-50 rounded-full">
          <X size={18} />
        </button>
        <NavContent />
      </div>
